...
rawdata_path <- '/home/cavin/jiangtao/spatial_data/stDCL/DLPFC/'
save_dir <- "/home/cavin/jiangtao/python/wave/DLPFC/_Seurat/images/"
libd_151507 <- Load10X_Spatial(data.dir = paste0(rawdata_path, sample.name),
                               filename = "filtered_feature_bc_matrix.h5",
                               assay = "Spatial",
                               slice = sample.name,
                               filter.matrix = T)
label <- read.table(file = paste0(rawdata_path, sample.name, "/metadata.tsv"))
...
write.csv(save_obj, file = paste0(save_dir, sample.name,"_label.csv"), 
          row.names = F)
}
