...
def train(name:str):
    ...
    label_path = os.path.join(dir,'cluterlabel_and_emb',name+'_spatial_cluster.csv')
    emb_path = os.path.join(dir,'cluterlabel_and_emb',name+'_spatial_emb.csv')
    ...
    adata.obs['domain'] = label.values[:, 0]
    # adata.obs['domain'] = adata.obs['domain'].astype('int')
    adata.obs['domain'] = adata.obs['domain'].astype('category')


    mask = ~adata.obs['domain'].isna()  # 保留 domain 非 NA 的细胞
    adata = adata[mask].copy()  # 创建新对象，保留有效细胞
    ...
    sc.pl.spatial(adata, color=["ground_truth", "domain"], title=['ground truth', 'SpatialPCA (ARI=%.2f)' % ARI_score],show=False)
    ...
    sc.pl.umap(adata, color=["domain", "ground_truth"], title=['SpatialPCA (ARI=%.2f)'%ARI_score, "Ground Truth"],palette=rainbow_hex[:cluster_num],show=False)
    ...
    sc.pl.paga_compare(adata, legend_fontsize=10, frameon=False, size=20,palette=rainbow_hex[:cluster_num], 
                    title=name+'_SpatialPCA', legend_fontoutline=2, show=False)
